...
class OllamaApiTest {
    ...
    @Test
    @DisplayName("BookAiService를 통한 Ollama 호출 테스트")
    void testBookAiServiceCallWithOllama() {
        ...
        // When
        try {
            String response = bookAiService.askAboutBooks(prompt);
            // Then
            log.info("[DEBUG_LOG] BookAiService (Ollama) Response: {}", response);
            assertThat(response).isNotEmpty();
        } catch (Exception e) {
            log.warn("[DEBUG_LOG] BookAiService (Ollama) Call Failed: {}", e.getMessage());
        }
    }
}
