    ...
    public void handleReviewAiSummeryEvent(ReviewAiSummaryEvent event) {
        ...
        if (!shouldGenerateSummary(summary)) {
            log.info("Conditions not met for book {}. Review count: {}, is_dirty: {}, is_generating: {}",
                    event.bookId(), summary.getReviewCount(), summary.getIsSummaryDirty(), summary.getIsGenerating());
            return;
        }

        // 중복 실행 방지 체크
        if (summary.getIsGenerating()) {
            log.info("Already generating summary for book {}. Skipping.", event.bookId());
            return;
        }
        ...
        try {
            ...
            if (needsRebuild) {
                ...
                log.info("Full rebuild needed for book {} ({} new reviews). Rebuilding all reviews.",
                        event.bookId(), reviewCount - lastSummarizedCount);

                List<BookReview> allReviews = bookReviewRepository.findAllByBookId(event.bookId());
                ...
                if (!newReviews.isEmpty()) {
                    ...
                    log.info("No new reviews to summarize for book {}.", event.bookId());
            ...
            log.error("Failed to generate summary for book {}: {}", event.bookId(), e.getMessage(), e);
        } finally {
            // 항상 플래그 해제 (에러 발생해도)
            try {
                ...
                log.error("Failed to reset is_generating flag for book {}: {}",
                        event.bookId(), saveException.getMessage());
            }
        }
    }
