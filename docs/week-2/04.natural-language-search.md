...
### 3.1. 커스텀 함수 등록
하이버네이트에서 `pgvector` 연산자를 인식할 수 있도록 `FunctionContributor`를 통해 벡터 유사도 계산 함수를 등록합니다.
...
### 3.2. 검색어의 벡터화 (Service)
사용자가 입력한 검색어(자연어)를 AI 모델을 사용하여 벡터로 변환합니다.
...
public Page<BookSearchResponse> search(BookSearchRequest request, Pageable pageable) {
    if (request.searchType() == SearchType.VECTOR) {
        ...
        return bookRepository.vectorSearch(pageable, request);
    }
    return bookRepository.search(pageable, request);
}
```

### 3.3. 벡터 검색 쿼리 작성 (QueryDSL)
등록된 커스텀 함수를 호출하여 유사도 점수를 계산하고 정렬하는 로직을 구현합니다.
...
public Page<BookSearchResponse> vectorSearch(Pageable pageable, BookSearchRequest request) {
    ...
    // 등록한 vector_cosine_similarity 함수 사용
    ...
    List<BookSearchResponse> results = queryFactory
            .select(Projections.constructor(BookSearchResponse.class,
                    book.id, book.isbn, book.title, ..., similarityTemplate))
            .from(book)
...
### 3.4. UI 결과 노출
응답 DTO에 포함된 `similarity` 점수를 활용하여 검색 결과 화면에 유사도(%)를 표시합니다.
