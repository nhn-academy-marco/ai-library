...
### 3.2. Spring AI Gemini 설정
`application.properties`에 Gemini API 키와 모델 정보를 설정합니다. `gemini-2.0-flash` 모델은 빠른 응답 속도를 제공하여 RAG 시스템에 적합합니다.

```properties
# Gemini API 설정
spring.ai.google.ai.api-key=YOUR_GEMINI_API_KEY
spring.ai.google.ai.chat.options.model=gemini-2.0-flash
```

### 3.2. AI 클라이언트 서비스 구현
Spring AI의 `ChatModel`을 사용하여 Gemini와 통신하는 서비스를 작성합니다.
...
### 3.3. 서비스 레이어 구현
`BookSearchService` 등에서 검색 결과와 프롬프트를 조합하여 AI에게 질의하는 로직을 추가합니다.
1. 사용자 질문 수신
2. (Week 2의) 벡터 검색 실행 → 도서 리스트 획득
3. (Week 3-02의) 컨텍스트 구성 및 프롬프트 완성
4. AI API 호출 및 응답 수신
5. 최종 답변 반환
