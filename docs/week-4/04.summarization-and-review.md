...
public class ReviewSummaryQueueService {
    ...
    public boolean enqueue(Long bookId) {
        ...
        if (lastEnqueueTime != null && (now - lastEnqueueTime) < DEDUP_WINDOW_MS) {
            log.debug("Duplicate summary request for book {}. Ignoring.", bookId);
            return false;
        }
        ...
        try {
            ...
            log.debug("Published summary task for book {} to RabbitMQ", bookId);
            return true;
        } catch (Exception e) {
...
**핵심 개념**
- `RabbitTemplate`: Spring AMQP가 제공하는 메시지 발행 템플릿
- `convertAndSend`: Java 객체를 JSON으로 변환해서 발행
...
**핵심 개념**
- `@RabbitListener`: 자동으로 메시지를 수신하는 리스너
- `concurrency="3-5"`: 3~5개의 Consumer 스레드가 병렬로 메시지 처리
- `Acknowledge-mode=auto`: 처리 성공 시 자동으로 메시지 확인, 실패 시 재시도
...
public class ReviewAiSummaryEventListener {
    ...
    public void handleReviewAiSummaryEvent(ReviewAiSummaryEvent event) {
        boolean enqueued = queueService.enqueue(event.bookId());
        if (enqueued) {
            log.info("Published AI summary task for book {} to RabbitMQ", event.bookId());
        } else {
            log.debug("Summary task for book {} already queued (dedup)", event.bookId());
        }
    }
...
**변경점**
- 기존: @Async로 직접 AI 요약 처리
- 변경: RabbitMQ에 메시지 발행 → Consumer가 처리
