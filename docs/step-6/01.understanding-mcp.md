...
> **📘 미션 가이드**: 점진적인 난이도로 MCP(Function Calling) 활용 미션을 수행합니다.
>
> 자세한 미션 시나리오는 [05.mission-scenarios.md](./05.mission-scenarios.md)를 참고하세요.

### 미션 난이도 개요

| 난이도 | 레벨 | 설명 | 예상 시간 |
|--------|------|------|----------|
| ⭐ | 입문 | 단일 Function 호출 | 30분 |
| ⭐⭐ | 초급 | 2-3개 Function 조합 | 1시간 |
| ⭐⭐⭐ | 중급 | 조건부 Function 호출 + 로직 | 2시간 |
| ⭐⭐⭐⭐ | 중상단계 | 복잡한 워크플로우 + 에러 처리 | 3시간 |
| ⭐⭐⭐⭐⭐ | 고급 | Multi-Agent 협업 | 4시간+ |

### 미션 목록

#### ⭐ 미션 1: 단일 Function 호출 - 도서 검색
**난이도**: 입문 | **예상 시간**: 30분
...
AI 동작:
1. search_books Function 호출
2. 검색 결과를 사용자에게 친절하게 설명
```

#### ⭐⭐ 미션 2: 복수 Function 조합 - 도서 검색 + 리뷰 조회
**난이도**: 초급 | **예상 시간**: 1시간

```
사용자: "이책 정보 알려줘: ISBN 9788960777331"

AI 동작:
1. get_book_info Function → 도서 기본 정보
2. get_book_reviews Function → 리뷰 조회
3. 종합 답변 제공
```

#### ⭐⭐⭐ 미션 3: 조건부 Function 호출 - 대출 가능 여부 확인
**난이도**: 중급 | **예상 시간**: 2시간

```
사용자: "내 주변에서 빌릴 수 있는 도서관 알려줘"

AI 동작:
1. search_books → ISBN 획득
2. get_region_code → 지역 코드 변환
3. get_loan_libraries → 대출 가능 도서관 조회
4. 지역별 정렬 제공
```

#### ⭐⭐⭐⭐ 미션 4: 복잡한 워크플로우 - 도서 등록 + 임베딩
**난이도**: 중상단계 | **예상 시간**: 3시간

```
사용자: "도서관정보나루에서 '자바의 정석' 검색해서 등록해줘"

AI 동작:
1. search_library_books → 도서관정보나루 검색
2. register_book → DB 저장
3. trigger_embedding → 임베딩 생성
4. report_progress → 진행 상황 피드백
```

#### ⭐⭐⭐⭐⭐ 미션 5: Multi-Agent 협업 - 신간 자동 수집
**난이도**: 고급 | **예상 시간**: 4시간+

```
사용자: "최근 1주일 신간 IT 도서 자동 수집해줘"

Agent 협업:
1. Collector Agent → 도서관정보나루 API 모니터링
2. Filter Agent → 중복 도서 필터링
3. Registrar Agent → 도서 등록
4. Embedding Agent → 임베딩 생성
5. Notifier Agent → 완료 알림
```
...
- [ ] Function Calling 메커니즘을 이해한다

### 실무 적용
- [ ] Spring AI Function을 정의할 수 있다
- [ ] ChatClient에 Function을 등록할 수 있다
- [ ] AI가 Function을 올바르게 호출하는지 확인할 수 있다

### 고급 기능
- [ ] A2A (AI to AI) 통신이 무엇인지 안다
- [ ] Multi-Agent 협업 시나리오를 설계할 수 있다
...
> **📋 미션 문서**: [05.mission-scenarios.md](./05.mission-scenarios.md)
>
> 각 미션별 상세 구현 가이드와 테스트 코드가 포함되어 있습니다.

미션을 순서대로 완료하면서 MCP에 대한 이해를 높이세요!
