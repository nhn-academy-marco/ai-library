...
1. searchBooks: 책 검색 (ISBN 확보)
2. checkLoanAvailability: ISBN으로 대출 가능 확인
3. getReviews: bookId로 리뷰 조회
4. 세 결과를 종합하여 응답 작성
""";
```
...
if (ISBN이 제공됨) {
    → searchBookByIsbn으로 정확한 검색
} else if (제목만 제공됨) {
    → searchBooks로 검색
    → 필요하면 checkLoanAvailability 추가
}
