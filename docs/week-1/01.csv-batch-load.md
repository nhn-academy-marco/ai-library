...
직접 구현할 때 다음 순서를 참고하세요.

1.  **데이터 모델 설계**: CSV 헤더와 매핑되는 DTO와 JPA 엔티티를 만듭니다.
2.  **파싱 로직 구현**: `Apache Commons CSV`와 같은 라이브러리를 사용하여 파일을 읽고, 각 레코드마다 이벤트를 발행합니다.
3.  **이벤트 리스너 작성**: 발행된 이벤트를 리스트(버퍼)에 담고, 파싱 완료 이벤트가 오면 실제 저장 서비스를 호출합니다.
4.  **배치 저장 서비스 구현**: 전달받은 리스트를 설정된 사이즈만큼 잘라서 `Repository.saveAll()`을 호출합니다.
