...
1.  **검색 서비스**: `@Transactional(readOnly = true)`를 적용하여 조회 성능을 높이고, 서비스 계층에서 레포지토리의 검색 기능을 호출합니다.
2.  **컨트롤러(Controller)**: 사용자의 검색어와 페이지 번호(`Pageable`)를 받아 서비스에 전달하고, 결과를 모델(Model)에 담아 뷰(View)로 전달합니다.
3.  **UI 템플릿(Template)**:
    - `default.html` 레이아웃을 상속받기 (`layout:decorate`)
    - 검색 폼(`form`) 구성 및 `name` 속성을 DTO 필드와 바인딩(Binding)
    - 결과 리스트(`th:each`) 출력 및 도서 이미지/제목 링크 추가
    - 공통 페이징 프래그먼트(Fragment) 호출
