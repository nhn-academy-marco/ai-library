...
1.  **Repository 인터페이스 확장**: 커스텀 인터페이스(`BookRepositoryCustom`)를 정의하여 QueryDSL 로직을 분리합니다.
2.  **QueryDSL 구현체 작성**: `BookRepositoryImpl` 클래스에서 실제 검색 로직을 작성합니다.
3.  **동적 쿼리 구성**:
    - `isbn` 검색: 정확히 일치(Equal)하는 데이터 조회
    - `keyword` 검색: 제목, 저자, 출판사 등 주요 필드에 키워드 포함 여부 조회 (`BooleanBuilder` 활용)
4.  **Projections 활용**: `@QueryProjection`을 사용하여 DB 조회 단계에서 즉시 DTO(`BookSearchResponse`)로 변환(Projection)하여 메모리 사용량을 최적화합니다.
5.  **페이징(Pagination) 처리**: `offset()`과 `limit()`을 사용하여 대량의 검색 결과 중 필요한 부분만 조회합니다.
